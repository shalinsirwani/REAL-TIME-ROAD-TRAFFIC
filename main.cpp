...
int main(void) {
    ...
    int intHorizontalLinePosition = 350;//(int)std::round((double)imgFrame1.rows * 0.35);
    int minv = 20;
    crossingLine[0].x = 0;
    ...
    crossingLine[2].x = 490;
    crossingLine[2].y = minv;

    crossingLine[3].x = imgFrame1.cols - 1;
    crossingLine[3].y = minv;
    ...
    while (capVideo.isOpened() && chCheckForEscKey != 27) {
        ...
        for (auto &convexHull : convexHulls) {
            ...
            if (possibleBlob.currentBoundingRect.area() > 1400 &&
                possibleBlob.dblCurrentAspectRatio > 0.5 &&
                possibleBlob.dblCurrentAspectRatio < 4.00 &&
        ...
        cv::line(imgFrame2Copy, crossingLine[0], crossingLine[2], SCALAR_RED, 2);

        cv::line(imgFrame2Copy, crossingLine[1], crossingLine[3], SCALAR_RED, 2);

        for(unsigned int i=0;i<blobs.size();i++)
        {
            int p2 = (int)blobs[i].centerPositions.size() - 2;
            int p1 = (int)blobs[i].centerPositions.size() - 1;
            if(blobs[i].blnStillBeingTracked == true && blobs[i].centerPositions.size() >=  2)
            {
                //std::cout<<i<<":- "<<blobs[i].centerPositions[p2].y<<" "<<blobs[i].centerPositions[p1].y<<std::endl;
                if(blobs[i].centerPositions[p2].y > crossingLine[0].y && blobs[i].centerPositions[p1].y < crossingLine[0].y)
                    {
                        carCount++;
                        drawCarCountOnImage(carCount, imgFrame2Copy);
                    }
                    bool ch = check(blobs[i].centerPositions[p2].y , blobs[i].centerPositions[p1].y);
                if(ch)
                {
                    carCount--;
                    drawCarCountOnImage(carCount , imgFrame2Copy);
...
void drawBlobInfoOnImage(std::vector<Blob> &blobs, cv::Mat &imgFrame2Copy) {
    ...
    for (unsigned int i = 0; i < blobs.size(); i++) {
        ...
        if (blobs[i].blnStillBeingTracked == true) {
            ...
            cv::Point p1 = blobs[i].centerPositions.back();
            int xs = p1.x;
            int ys = p1.y;
            std::string str = std::to_string(ys) + ",," + std::to_string(xs);
            cv::putText(imgFrame2Copy, str , blobs[i].centerPositions.back(), intFontFace, dblFontScale, SCALAR_GREEN, intFontThickness);
        }
    }
}
